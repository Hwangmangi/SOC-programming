...
int main(void)
{
	...
	while(TRUE)
	{
		...
		//SegReg=((RecvBuffer[2] & 0x3f) << 24) | (0xa << 20) | ((RecvBuffer[1] & 0x7f) << 12) | (0xa << 8) | RecvBuffer[0];
		SegReg=((RecvBuffer[2] & 0x3f) << 24) + (0xa << 20) + ((RecvBuffer[1] & 0x7f) << 12) + (0xa << 8) | (RecvBuffer[0] & 0x7f);

		/*
		SegReg=0;
		SegReg=(SegReg|(u8)RecvBuffer[2])<<24;
		SegReg=(SegReg|0x0a)<<20;
		SegReg=(SegReg|(u8)RecvBuffer[1])<<12;
		SegReg=(SegReg|0x0a)<<8;
		SegReg=(SegReg|(u8)RecvBuffer[0]);
		*/
...
void WriteTLCDReg(char *pRegVal, int val) //pRegVal은 16Byte의 배열
{
	...
	for(i = 0; i < 8; i++)
	{
		//temp=((u8)(val>>(28-i*4)) & 0x0F)|0x30;
		temp=((val>>(28-i*4)) & 0x0F)+0x30;
		// Coding here!
		pRegVal[i] = temp;
		pRegVal[i+8] = 0x20;
	}
